...
public class OBJLoader {
    ...
    private static void processVertex(String[] vertexData, List<Integer> indices, List<Vector2f> textureCoords, List<Vector3f> normals, float[] textureCoordsArray, float[] normalsArray) {
        int currentVertexPointer = Integer.parseInt(vertexData[0]) - 1;
        indices.add(currentVertexPointer);
        Vector2f currentTexCoord = textureCoords.get(Integer.parseInt(vertexData[1]) - 1);
        textureCoordsArray[currentVertexPointer*2] = currentTexCoord.x;
        textureCoordsArray[currentVertexPointer*2+1] = currentTexCoord.y; // 1 - Y may be needed
